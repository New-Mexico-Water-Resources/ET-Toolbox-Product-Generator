...
class VNP09GAGranule(VIIRSGranule):
    ...
    def get_M_band(
        ...
        if product_filename is not None and exists(product_filename):
            logger.info(f"loading VIIRS M{band}: {cl.file(product_filename)}")
            image = Raster.open(product_filename)
        else:
            image = self.dataset(
                self.filename,
    ...
    def get_I_band(
        ...
        if product_filename is not None and exists(product_filename):
            logger.info(f"loading VIIRS I{band}: {cl.file(product_filename)}")
            image = Raster.open(product_filename)
        else:
            image = self.dataset(
                self.filename,
    ...
    def NDVI(
        ...
        for tile in tiles:
            ...
            if NDVI is None:
                ...
                NDVI = raster.where(np.isnan(NDVI), projected_NDVI, NDVI)
    ...
    def albedo(
        ...
        for tile in tiles:
            ...
            if albedo is None:
                ...
                albedo = raster.where(np.isnan(albedo), projected_albedo, albedo)
