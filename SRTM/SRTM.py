...
from raster import Raster, RasterGeometry, raster, RasterGrid
...
class SRTMGranule:
    ...
    def elevation_m(self) -> Raster:
        ...
        data = raster.where(data == data.nodata, np.nan, data.astype(np.float32))
        data.nodata = np.nan
...
class SRTM(LPDAACDataPool):
    ...
    #         else:
    #             result = raster.where(np.isnan(result), image_projected, result)
    #
    #     return result
    ...
    def swb(self, geometry: RasterGeometry) -> Raster:
        ...
        result = raster.where(np.isnan(result), 1, result)
    ...
    def elevation_m(self, geometry: RasterGeometry) -> Raster:
        ...
        for tile in tiles:
            ...
            if result is None:
                ...
                result = raster.where(np.isnan(result), elevation_projected, result)
