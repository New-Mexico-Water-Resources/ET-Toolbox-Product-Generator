...
class LandsatL2C2Granule(object):
    ...
    def SR(
        ...
        if apply_cloud:
            image = raster.where(self.cloud, np.nan, image)

        if apply_water:
            image = raster.where(self.water, np.nan, image)
        elif apply_land:
            image = raster.where(self.water, image, np.nan)

        image = raster.where(image < self.MTL.SR_DN_min(band), np.nan, image)
        image = raster.where(image > self.MTL.SR_DN_max(band), np.nan, image)
        image = image * self.MTL.SR_mult(band) + self.MTL.SR_add(band)
        image = raster.where(image < self.MTL.SR_min(band), np.nan, image)
        image = raster.where(image > self.MTL.SR_max(band), np.nan, image)
    ...
    def get_ST(
        ...
        image = raster.where(image < self.ST_DN_min, np.nan, image)
        image = raster.where(image > self.ST_DN_max, np.nan, image)
        image = image * self.ST_mult + self.ST_add
        image = raster.where(image < self.ST_min, np.nan, image)
        image = raster.where(image > self.ST_max, np.nan, image)
    ...
    def water(self):
        ...
        self._water = raster.where((self.QA >> 7) & 1, True, False)
